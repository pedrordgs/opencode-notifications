...
## Adding New Wayland Compositor Support

To add support for a new Wayland compositor:

1. Create a new class implementing `FocusDetector` in `src/focus.ts`
2. Implement `init()` to capture the terminal's window ID
3. Implement `isTerminalFocused()` to check if that ID is currently focused
4. Add detection logic in `createFocusDetector()` using environment variables
5. Update README.md with requirements and documentation

Example for a hypothetical compositor:

```typescript
class MyCompositorFocusDetector implements FocusDetector {
  readonly name = "MyCompositor"

  async init(): Promise<string | null> {
    if (!(await commandExists("myctl"))) {
      return null
    }
    return runCommand("myctl get-focused-window")
  }

  async isTerminalFocused(terminalId: string): Promise<boolean> {
    const current = await runCommand("myctl get-focused-window")
    return current === terminalId
  }
}
```

Then in `createFocusDetector()`:

```typescript
if (process.env.MY_COMPOSITOR_SOCK) {
  return new MyCompositorFocusDetector()
}
```
